...
class ProfilesViewProfile extends HtmlView
{
	...
	public function display($tpl = null)
	{
		...
		if ($active)
		{
			...
			if (strpos($currentLink, 'view=profile') && strpos($currentLink, '&id=' . (string) $item->id))
			{
				...
				elseif ($layout = $item->params->get('profile_layout'))
				{
	...
	protected function _prepareDocument()
	{
		...
		{
			$this->params->def('page_heading', Text::_('COM_PROFILES_PROFILE'));
		}
		$title = $this->params->get('page_title', $sitename);

		// If the menu item does not concern this contact
		if ($menu && ($menu->query['option'] !== 'com_profiles' || $menu->query['view'] !== 'profile' || $id != $item->id))
		{
			if ($item->name)
			{
				$title = $item->name;
